...
function CategoryBadge({ name }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: colors.bg, color: colors.text }}>
      <Tag size={10} />
      {name}
    </span>
  );
    ...
    >
      ...
      <div className="p-8 md:p-10 flex flex-col justify-center items-center text-center">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <Link to={`/blog/${post.id}`} className="inline-flex items-center gap-2 font-bold text-sm transition-all group" style={{ color: '#E07B39' }}>
            {tx.readArticle}
...
function PostCard({ post, delay = 0, tx }) {
  return (
    <motion.article
    ...
    <>
      ...
      <section className="py-14 md:py-28 bg-cream">
        <div className="container-custom">
          ...
          {featured && (
            <div className="mb-20">
              <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={vp} transition={{ duration: 0.5 }} className="text-center mb-8">
                <span className="section-label">{tx.latestArticle}</span>
              </motion.div>
              <FeaturedCard post={featured} tx={tx} />
            </div>
          )}
          ...
          <div className="mb-12">
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={vp} transition={{ duration: 0.5 }} className="text-center mb-12">
              <span className="section-label">{tx.moreArticles}</span>
              <h2 className="section-title text-2xl md:text-3xl">{tx.latestFromBlog}</h2>
            </motion.div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-7">
              {rest.map((post, i) => <PostCard key={post.id} post={post} delay={i * 0.07} tx={tx} />)}
            </div>
          </div>
          ...
          >
            <span className="section-label" style={{ color: '#E07B39' }}>{tx.newsletterLabel}</span>
            <h3 className="text-3xl font-extrabold text-white mb-3 mt-1">{tx.newsletterTitle}</h3>
            <p className="text-white/65 mb-7 max-w-md mx-auto text-sm">{tx.newsletterSubtitle}</p>
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input type="email" placeholder={tx.newsletterPlaceholder} className="flex-1 px-5 py-3.5 rounded-full text-sm text-gray-800 focus:outline-none" style={{ background: 'rgba(255,255,255,0.95)' }} />
              <button className="btn-orange shrink-0 px-6">{tx.subscribeFree}</button>
            </div>
          </motion.div>
        </div>
      </section>
