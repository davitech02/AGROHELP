...
function SocialBtn({ href, label, bg, children }) {
  return (
    <a href={href} aria-label={label}
      ...
      <svg viewBox="0 0 24 24" style={{ width: 13, height: 13 }} xmlns="http://www.w3.org/2000/svg">{children}</svg>
    </a>
  );
}
...
export default function Contact() {
  ...
  const tx = t[language].contactPage;
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden" style={{ height: '320px' }}>
        <img src="https://images.unsplash.com/photo-1521737711867-e3b97375f902?w=1600&h=640&fit=crop&q=80" alt="" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(27,67,50,0.88) 0%, rgba(27,67,50,0.70) 100%)' }} />
        <div className="relative h-full flex flex-col items-center justify-center text-center px-6 pt-16">
          <motion.h1 initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}
        ...
        <div className="container-custom">
          ...
          >
            <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
              ...
              {/* LEFT */}
              <div className="p-8">
                <h2 className="text-2xl font-extrabold mb-4" style={{ color: '#1B4332' }}>{tx.getInTouch}</h2>
                <p className="text-gray-500 text-sm mb-8 leading-relaxed">{tx.getInTouchDesc}</p>

                <div className="space-y-6">
                  {contactRows.map(({ Icon, label, lines, hrefs, iconBg, iconColor }, i) => (
                    <div key={i} className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ backgroundColor: iconBg }}>
                        <Icon size={18} style={{ color: iconColor }} />
                      </div>
                      <div>
                        <p className="font-semibold text-sm mb-0.5" style={{ color: '#1B4332' }}>{label}</p>
                        {lines.map((line, j) =>
                          hrefs && hrefs[j] ? (
                            <a key={j} href={hrefs[j]} className="block text-gray-500 text-sm hover:text-[#E07B39] transition-colors">{line}</a>
                          ) : (
                ...
                <a href="https://wa.me/33746332937" target="_blank" rel="noopener noreferrer"
                ...
                <p className="text-sm font-semibold text-gray-600 mb-4 mt-2">{tx.followSocial}</p>
                <div className="flex gap-2.5">
                  ...
                  <SocialBtn href="#" label="Instagram" bg="#E1306C">
                    <g fill="none" stroke="white" strokeWidth="1.5"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><circle cx="12" cy="12" r="4" /></g>
                    <circle fill="white" cx="17.5" cy="6.5" r="1.3" />
                  </SocialBtn>
                  <SocialBtn href="#" label="Twitter" bg="#1DA1F2">
                    <path fill="white" d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
                  </SocialBtn>
                  <SocialBtn href="#" label="YouTube" bg="#FF0000">
                    <path fill="white" d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58 2.78 2.78 0 0 0 1.95 1.95C5.12 20 12 20 12 20s6.88 0 8.59-.47a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" />
              ...
              {/* RIGHT */}
              <div className="p-8">
                <h2 className="text-2xl font-extrabold mb-4" style={{ color: '#1B4332' }}>{tx.sendMessage}</h2>
                <p className="text-gray-400 text-sm mb-8 leading-relaxed">{tx.sendMessageDesc}</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4332' }}>
                        {tx.fullName} <span style={{ color: '#E07B39' }}>*</span>
                      </label>
                      <input type="text" name="name" value={formData.name} onChange={handleChange} required className={inputCls} placeholder={tx.namePh} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4332' }}>{tx.company}</label>
                      <input type="text" name="company" value={formData.company} onChange={handleChange} className={inputCls} placeholder={tx.companyPh} />
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4332' }}>{tx.phone}</label>
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className={inputCls} placeholder={tx.phonePh} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4332' }}>
                        {tx.email} <span style={{ color: '#E07B39' }}>*</span>
                      </label>
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4332' }}>
                      {tx.subject} <span style={{ color: '#E07B39' }}>*</span>
                    </label>
                    <input type="text" name="subject" value={formData.subject} onChange={handleChange} required className={inputCls} placeholder={tx.subjectPh} />
                  </div>

                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4332' }}>
                      {tx.message} <span style={{ color: '#E07B39' }}>*</span>
                    </label>
                    <textarea name="message" value={formData.message} onChange={handleChange} required rows="5"
                      className={`${inputCls} resize-none`} placeholder={tx.messagePh} />
                  </div>
                  ...
                  <button type="submit" disabled={loading}
                    ...
                    {loading ? tx.sendingBtn : tx.sendBtn}
                  </button>
                </form>
              </div>
            </div>
